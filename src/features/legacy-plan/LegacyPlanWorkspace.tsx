...
export type LegacyPlanWorkspaceProps = {
  onUnsavedChange?: (hasUnsavedChanges: boolean) => void;
};

export function LegacyPlanWorkspace({
  onUnsavedChange,
}: LegacyPlanWorkspaceProps) {
  ...
  const onModifyPlan = useCallback(() => {
    ...
    if (!selectedRawText) {
      library.setError(
        "Cannot modify the editable plan because its .rob text is unavailable.",
      );
      return;
    }
    try {
      editor.replaceEditedPlan(rotateRobPlanBy180(selectedRawText));
    } catch (cause) {
      console.error("Failed to modify plan", cause);
      library.setError("Unable to modify the plan at this time.");
    }
  }, [data, editor, library, selectedRawText]);
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col gap-4 p-4">
      <ViewerToolbar
        ...
        modifyDisabled={!data || !selectedRawText}
      />
      {library.error && (
        <div
          role="alert"
      ...
      {!viewedData && (
        <div className="flex min-h-[420px] flex-1 items-center justify-center rounded-md border border-dashed border-zinc-800 bg-zinc-900/40">
          ...
          layerSelector={
            <label className="flex min-w-[250px] flex-col gap-1 text-xs text-zinc-500">
              ...
              <select
                ...
                className="cursor-pointer rounded-md border border-zinc-700 bg-zinc-950 px-2 py-1.5 text-xs text-zinc-200 outline-none focus:border-zinc-500 focus:ring-2 focus:ring-amber-400/40"
              >
