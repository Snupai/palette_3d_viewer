...
- Each coordinate line holds x, y, rotation (0/90/180/270), package count, and dx/dy encoding the blue-line side or corner; a multi-package line is a single grip, so all its boxes share the same `placeX`/`placeY`.
- Bottom-face Z of a layer is the sum of package heights below plus Zwischenlagen (3 mm each, `ZWISCHENLAGE_HEIGHT_MM`, matching the robot's `Dicke_ZwLagen`); pallet height is excluded.
